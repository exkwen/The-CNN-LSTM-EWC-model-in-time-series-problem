...
# set a suitable dataset from CNN-LSTM to read
...
class CnnLstm(nn.Module):
    ...
    def __init__(self, args):
        ...
        self._conv = nn.Conv1d(in_channels=args.seq_len, out_channels=args.input_size, kernel_size=args.conv_kernel,
                               padding=64)
        self._tanh = nn.Tanh()
        self._max_pool = nn.MaxPool1d(kernel_size=args.pool_size)

        # lstm architecture
        self.lstm = nn.LSTM(input_size=args.input_size, hidden_size=args.hidden_size, num_layers=args.num_layers,
...
def arg_default():
    ...
    # data preprocessing
    parser.add_argument('--seq_len', type=int, default=4, help='the collection information of a few days')
    ...
    parser.add_argument('--input_size', default=64, type=int, help='Enter the dimension of the feature')
    ...
    parser.add_argument('--save_file', default='model/CNN_LSTM_Stock.pkl', help='used to save the current model')
...
def collect_inform(inform):
    filename = 'experiment_results/CNN-LSTM-EWC.csv'
...
def cnn_lstm(filename, op_name, time_step, le_rate, learned_percent):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        inform = [filename, op_name, "StepLR:1", learned_percent, args.lr, epoch, average_train_loss,
                  mean_absolute_error]
...
if __name__ == '__main__':
    ...
    cnn_lstm("dataset/Stock_1.csv", "Adam", 1, 0.001, 0.99)
